...
/**
 * Clase que gestiona las modalidades. CRUD
 * @author kadet
 *
 */

class ClaseGetModalidades {
...
//         $this->id_tienda = $id_tienda;
    ...
    /**
     * OBTENER DETALLE
     * obtiene las modalidades de esta tienda
     */
    
    public static function consultaSinFormatoPorTienda($idTienda){
        $conexion = new conexion();//objeto conexion
        $conexion->exec("SET NAMES 'utf8'");
        $consulta = $conexion->prepare("
                        SELECT SQL_CALC_FOUND_ROWS * FROM generica_modalidades A
                        INNER JOIN definicion_modalidades B on A.id_modali = B.id_modali
                        WHERE A.id_tienda = :idTienda ORDER BY A.id_modali ASC
                          ");//uso la constante TABLA
        $consulta->execute(array(':idTienda' => $idTienda));
        $consulta->execute();
